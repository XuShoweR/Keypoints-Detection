...
def convert_train(output_dir, val_per=0.015, all_splits=config.SPLITS, file_idx_start=0):
    ...
    for cat in config.CATEGORIES:
        ...
        # tf_filename = tf_filename[:-21]
        # if not os.path.exists(tf_filename):
        #     os.mknod(tf_filename)
...
if __name__ == '__main__':
    ...
    # convert_train(config.RECORDS_DATA_DIR, val_per=0., all_splits=config.TRAIN_DIR)
    ...
    convert_test(config.TEST_RECORDS_STAGE2, splits=['local_test'])
    # convert_train(config.RECORDS_DATA_DIR, val_per=0., all_splits=config.TRAIN_DIR)
