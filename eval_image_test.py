...
tf.app.flags.DEFINE_string(
    'data_dir', '/media/roots/keypoints_detection/Datasets/tfrecords_test',#tfrecords_test tfrecords_test_stage1_b tfrecords_test_stage2
...
def input_pipeline_test(model_scope=FLAGS.model_scope):
    file_name = "./0a0c265cf33e3399297b30d795853ae6.jpg"
    # image_data = tf.gfile.FastGFile(filename, 'rb').read()
    images = cv2.imread(file_name)
    shape = misc.imread(file_name).shape
    shape = tf.cast(tf.expand_dims(tf.stack([shape[0], shape[1], shape[2]], axis=0), axis=1), dtype=tf.int64)
    classid = tf.constant([0], tf.int64)
    offsets = tf.constant([0, 0], tf.int64)
    # images, shape, file_name, classid, offsets = tf.train.batch([image_data, shape, file_name, classid, offsets],
    #                                                              dynamic_pad=False,
    #                                                              batch_size=1,
...
def eval_each(model_fn, model_dir, model_scope, run_config):
    ...
    pred_results = fashionAI.predict(input_fn=lambda : input_pipeline_test(model_scope), hooks=[logging_hook], checkpoint_path=train_helper.get_latest_checkpoint_for_evaluate_(model_dir, model_dir))
    #tf.logging.info()
