...
# https://github.com/GeorgeSeif/Semantic-Segmentation-Suite/blob/master/frontends/se_resnext.py
...
def xt_cascaded_pyramid_net(inputs, output_channals, heatmap_size, istraining, data_format, net_depth=50):
    ...
    with tf.variable_scope('global_net', 'global_net', values=pyramid_laterals):
        ...
        for ind, lateral in enumerate(pyramid_laterals):
            ...
            # if ind == 3:
            #     outputs = tf.image.resize_bilinear(outputs, [heatmap_size, heatmap_size], name='global_heatmap_p{}'.format(pyramid_len - ind))
            # # outputs_size = tf.shape(outputs)[-2]
            # else:
            #     outputs_channels = outputs.shape[-1]
            #     upsample_stride = 2 ** (3 - ind)
            #     # conv2d_transpose
            #     outputs = tf.layers.conv2d_transpose(outputs, outputs_channels, 2 * upsample_stride, (upsample_stride, upsample_stride), padding='same', name='global_heatmap_p{}'.format(pyramid_len - ind))
