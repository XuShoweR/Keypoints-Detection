...
tf.app.flags.DEFINE_string(
    'data_dir', '/media/roots/keypoints_detection/Datasets/tfrecords',#'/media/rs/0E06CD1706CD0127/Kapok/Chi/Datasets/tfrecords',
...
def keypoint_model_fn(features, labels, mode, params):
    ...
    print("pred_outputs",pred_outputs)
    ...
    print("target_leng:",len(targets_list))
    print("pred_out:",len(pred_outputs))
    ...
    if params['use_ohkm']:
        ...
        for pred_ind in list(range(len(pred_outputs) - 1)):
            # 看一下这个targets_list的shape 是一个channel对应一个关键点？
            # ans: targets_list 是7x7 9x9 …… 的高斯
...
def main(_):
    ...
    for m in model_to_train:
        ...
        sub_loop(keypoint_model_fn, m, detail_params[m]['model_dir'], run_config, detail_params[m]['train_epochs'], detail_params[m]['epochs_per_eval'], detail_params[m]['lr_decay_factors'], detail_params[m]['decay_boundaries'], None, detail_params[m]['checkpoint_exclude_scopes'], detail_params[m]['checkpoint_model_scope'], detail_params[m]['ignore_missing_vars'])
